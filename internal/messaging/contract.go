...
// ReplySubject maps a command type to its reply subject.
func ReplySubject(cmdType string) string {
	if cmdType == TypeRefund {
		return SubjectRefundReply
	}
	return SubjectCaptureReply
}
