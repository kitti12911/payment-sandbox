// Package relay publishes pending payment_outbox rows to NATS and marks them
// sent, giving the participant transactional-outbox delivery guarantees.
package relay
...
// Relay drains the payment outbox to the message bus.
type Relay struct {
	db        *orm.DB
	bus       *async.Bus
	...
	for {
		...
		case <-ticker.C:
			if err := r.drain(ctx); err != nil {
				slog.ErrorContext(ctx, "outbox relay drain failed", "error", err)
			}
		}
	}
}
...
func (r *Relay) drain(ctx context.Context) error {
	...
	for i := range rows {
		...
		var reply messaging.Reply
		if err := json.Unmarshal(row.Payload, &reply); err != nil {
			return fmt.Errorf("unmarshal outbox payload %s: %w", row.ID, err)
		}

		if err := r.bus.Publish(ctx, row.Topic, reply); err != nil {
			return fmt.Errorf("publish outbox %s: %w", row.ID, err)
		}

		if _, err := r.db.IDB(ctx).NewUpdate().
			Model((*database.Outbox)(nil)).
